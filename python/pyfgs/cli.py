...
def _FnaFormatter(header, seq, genes) -> bytes:
    return b"".join(
        b">%s_FGS_%d\n%s\n" % (header, n, gene.sequence()) for n, gene in enumerate(genes, start=1)
    )
...
def _BedFormatter(header, seq, genes) -> bytes:
    ...
    for n, gene in enumerate(genes, start=1):
        ...
        if muts:
            mut_info = [
                f"pos={m.pos};type={m.mut_type};codon={m.codon_idx};note={m.annotation}".encode()
                for m in muts
            ]
...
def _GffFormatter(header, seq, genes) -> bytes:
    ...
    for n, gene in enumerate(genes, start=1):
        ...
        if muts:
            notes = b",".join(
                b"Frameshift %b at pos %d (codon %d)"
            ...
            lines.append(
                b"%s\tpyfgs\tCDS\t%d\t%d\t%.2f\t%s\t0\tID=%s;inference=ab initio prediction:pyfgs\n"
...
def _VcfFormatter(header, seq, genes) -> bytes:
    ...
    for n, gene in enumerate(genes, start=1):
        ...
        for m in muts:
            ...
            info = (
                b"TYPE=frameshift_%s;GENE=%s;CODON=%d;ANN=%s|frameshift_variant|HIGH|%s|%s|transcript|%s|protein_coding|%d/1|%s"
...
def get_parser():
    ...
    in_group.add_argument(
        "-r", "--reads", action="store_true", help="Force FASTQ parsing (Overrides auto-detection)"
    )
    in_group.add_argument(
        "-w",
        "--whole-genome",
    ...
    out_group.add_argument(
        "--faa", nargs="?", const="-", default=None, metavar="PATH", help="Output protein FASTA"
    )
    out_group.add_argument(
        "--fna", nargs="?", const="-", default=None, metavar="PATH", help="Output nucleotide FASTA"
    )
    out_group.add_argument(
        "--bed", nargs="?", const="-", default=None, metavar="PATH", help="Output BED6+1 format"
    )
    out_group.add_argument(
        "--gff", nargs="?", const="-", default=None, metavar="PATH", help="Output GFF3 format"
    )
    out_group.add_argument(
        "--vcf", nargs="?", const="-", default=None, metavar="PATH", help="Output VCF v4.2 format"
    )
    ...
    other_group.add_argument(
        ...
        "--threads",
        type=int,
        default=_get_optimal_threads(),
        metavar="",
        help="Number of threads (default: optimal)",
    )
    other_group.add_argument(
        "-v", "--version", action="version", version="pyfgs 0.1.0", help="Print version and exit"
    )
...
def main():
    ...
    active_outputs = {
        fmt: getattr(args, fmt) for fmt in formats_map if getattr(args, fmt) is not None
    }
